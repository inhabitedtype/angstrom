...
module Alcotest = struct
  ...
  let bigstring = Alcotest.testable (fun fmt _bs -> Fmt.pf fmt "<bigstring>") ( = )
end
...
let check ?size f p is =
  ...
  let state =
    List.fold_left
      (fun state chunk -> feed state (`String chunk))
      (parse ?initial_buffer_size:size p)
      is
  in
...
let check_ok ?size ~msg test p is r =
  let r = Ok r in
  check ?size (fun result -> Alcotest.(check (result test string)) msg r result) p is
;;

let check_fail ?size ~msg p is =
  let r = Error "" in
  check ?size (fun result -> Alcotest.(check (result reject pass)) msg r result) p is
;;
...
let little_endian =
  let module E =
    Endian (struct
      ...
      let set_float bs off f =
        Bigstringaf.unsafe_set_int32_le bs off (Int32.bits_of_float f)
      ;;

      let set_double bs off d =
        Bigstringaf.unsafe_set_int64_le bs off (Int64.bits_of_float d)
      ;;
    end)
  in
  E.tests (module LE)
...
let big_endian =
  let module E =
    Endian (struct
      ...
      let set_float bs off f =
        Bigstringaf.unsafe_set_int32_be bs off (Int32.bits_of_float f)
      ;;

      let set_double bs off d =
        Bigstringaf.unsafe_set_int64_be bs off (Int64.bits_of_float d)
      ;;
    end)
  in
  E.tests (module BE)
...
let incremental =
  [ ( "within chunk boundary"
    , `Quick
    , fun () ->
        check_s
          ~msg:"string on each side of 2 inputs"
          (string "this" *> string "that")
          [ "this"; "that" ]
          "that";
        check_s
          ~msg:"string on each side of 3 inputs"
          (string "thi" *> string "st" *> string "hat")
          [ "thi"; "st"; "hat" ]
          "hat";
        check_s
          ~msg:"string straddling 2 inputs"
          (string "thisthat")
          [ "this"; "that" ]
          "thisthat";
        check_s
          ~msg:"string straddling 3 inputs"
          (string "thisthat")
          [ "thi"; "st"; "hat" ]
          "thisthat" )
  ; ( "peek_char and empty chunks"
    , `Quick
    , fun () ->
        let decoder len =
        ...
        in
        check_s
          ~msg:"empty input multiple times and peek_char"
          (decoder 0xFF)
          [ "Whole Lotta Love"; ""; ""; "" ]
          "Whole Lotta Love" )
  ; ( "across chunk boundary"
    ...
    , fun () ->
        check_s
          ~size:4
          ~msg:"string on each side of 2 chunks"
          (string "this" *> string "that")
          [ "this"; "that" ]
          "that";
        check_s
          ~size:3
          ~msg:"string on each side of 3 chunks"
          (string "thi" *> string "st" *> string "hat")
          [ "thi"; "st"; "hat" ]
          "hat";
        check_s
          ~size:4
          ~msg:"string straddling 2 chunks"
          (string "thisthat")
          [ "this"; "that" ]
          "thisthat";
        check_s
          ~size:3
          ~msg:"string straddling 3 chunks"
          (string "thisthat")
          [ "thi"; "st"; "hat" ]
          "thisthat" )
  ; ( "across chunk boundary with commit"
    , `Quick
    , fun () ->
        check_s
          ~size:4
          ~msg:"string on each side of 2 chunks"
          (string "this" *> commit *> string "that")
          [ "this"; "that" ]
          "that";
        check_s
          ~size:3
          ~msg:"string on each side of 3 chunks"
          (string "thi" *> string "st" *> commit *> string "hat")
          [ "thi"; "st"; "hat" ]
          "hat" )
  ]
;;
...
let count_while_regression =
  [ ( "proper position set after count_while"
    , `Quick
    , fun () ->
        check_s
          ~msg:"take_while then eof"
          (take_while (fun _ -> true) <* end_of_input)
          [ "asdf"; "" ]
          "asdf";
        check_s
          ~msg:"take_while1 then eof"
          (take_while1 (fun _ -> true) <* end_of_input)
          [ "asdf"; "" ]
          "asdf" )
  ]
;;
...
let choice_commit =
  [ ( ""
    ...
    , fun () ->
        let p =
          choice [ string "@@" *> commit *> char '*'; string "@" *> commit *> char '!' ]
        in
