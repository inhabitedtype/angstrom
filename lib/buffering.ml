...
let ensure t to_copy =
  if trailing_space t < to_copy
  then if writable_space t >= to_copy then compress t else grow t to_copy
;;
...
let for_reading { buf; off; len } = Bigstringaf.sub ~off ~len buf
...
let of_unconsumed { Unconsumed.buf; off; len } = { buf; off; len }
